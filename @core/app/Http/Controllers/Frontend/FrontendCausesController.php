...
class FrontendCausesController extends Controller
{
    ...
    public function donations_single($slug)
    {
        ...
        $follow = '';
        if (!is_null($user)){
            $follow = UserFollow::where(['campaign_owner_id'=>$user->id, 'user_type'=>'user'])
                ->orWhere(['campaign_owner_id'=>$user->id,'user_type'=>'admin'])->first();
        }
...
//        $donation_logs = CauseLogs::findOrFail($id);
    ...
    {
        $all_donations = Cause::where(['categories_id' => $id,'status' => 'publish'])->get();
    ...
    public function donation_search_page(Request $request)
    {
        ...
        if($request->search == ''){
            return redirect()->route('homepage')->with(['type'=>'danger', 'msg'=>'Search Field is required']);
        }


        $all_donations = Cause::Where('title', 'LIKE', '%' . $request->search . '%')
